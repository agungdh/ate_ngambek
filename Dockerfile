...
RUN set -eux; \
  ...
  docker-php-ext-install -j"$(nproc)" \
    bcmath intl gd zip mbstring opcache pdo_pgsql curl \
    xml dom simplexml xmlreader xmlwriter; \
  pecl install redis; \
  docker-php-ext-enable redis; \
  \
  # bersih-bersih build deps
  rm -rf /tmp/pear; \
...
COPY --from=builder-php --chown=www-data:www-data /var/www/html /var/www/html
